      ...
      try {
        parallel (
          e2e: {
            ...
            sh "export IMAGE_TAG=${commonStages.IMAGE_TAG} && docker-compose -p e2e up -d && docker-compose -p e2e run --rm --entrypoint=npm -e BABEL_ENV=test -e BUILDTYPE=vagovdev vets-website --no-color run test:puppeteer:docker"
          },
